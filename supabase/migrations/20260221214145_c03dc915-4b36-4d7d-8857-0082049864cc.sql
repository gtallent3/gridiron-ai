...
CREATE POLICY "Users can view own drafts"
  ON public.mock_drafts FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create own drafts"
  ON public.mock_drafts FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own drafts"
  ON public.mock_drafts FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view picks for own drafts"
  ON public.mock_draft_picks FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.mock_drafts WHERE id = draft_id AND user_id = auth.uid()));
CREATE POLICY "Users can insert picks for own drafts"
  ON public.mock_draft_picks FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.mock_drafts WHERE id = draft_id AND user_id = auth.uid()));
